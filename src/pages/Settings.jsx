    ...
    <div className="settings-page">
      ...
      <div className="settings-content" ref={formRef}>
        ...
        {activeSection === 'rate-limit' && (
          <>
            <div className="settings-section-header">
              <div className="settings-section-icon" style={{ background: 'rgba(168, 85, 247, 0.1)' }}>
            ...
            <p className="settings-section-desc">
              Control how many emails are sent per time window to maintain deliverability and avoid spam filters.
            </p>
            ...
            <div className="settings-group">
              <div className="settings-row">
                <div>
                  ...
                  <div className="settings-row-desc">Maximum emails to send in each time window</div>
                </div>
                <select
                  ...
                  onChange={(e) => setLocalSettings(prev => ({
                    ...prev,
                    rateLimit: { ...prev.rateLimit, emailsPerWindow: parseInt(e.target.value) }
                  }))}
                >
              ...
              <div className="settings-row">
                <div>
                  ...
                  <div className="settings-row-desc">Time window for rate limiting</div>
                </div>
                <select
                  className="settings-select"
                  ...
                  onChange={(e) => setLocalSettings(prev => ({
                    ...prev,
                    rateLimit: { ...prev.rateLimit, windowMinutes: parseInt(e.target.value) }
                  }))}
                >
            ...
            <div style={{ 
              ...
              <span style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
                Current rate: <strong>{localSettings?.rateLimit?.emailsPerWindow || 2}</strong> emails every <strong>{localSettings?.rateLimit?.windowMinutes || 15}</strong> minutes = ~{Math.round((localSettings?.rateLimit?.emailsPerWindow || 2) * (60 / (localSettings?.rateLimit?.windowMinutes || 15)))} emails/hour
              </span>
            </div>

            <button className="btn btn-primary" onClick={handleSaveRateLimit} disabled={saving}>
        ...
        {activeSection === 'business-hours' && (
          <>
            <div className="settings-section-header">
              <div className="settings-section-icon" style={{ background: 'rgba(59, 130, 246, 0.1)' }}>
            ...
            <p className="settings-section-desc">
              Define when emails can be sent. Emails scheduled outside these hours will be shifted to the next available window.
            </p>
            ...
            <div className="settings-group">
              <div className="settings-row">
                <div>
                  ...
                  <div className="settings-row-desc">Earliest time to send emails</div>
                </div>
                <select
                  className="settings-select"
                  ...
                  onChange={(e) => setLocalSettings(prev => ({
                    ...prev,
                    businessHours: { ...prev.businessHours, startHour: parseInt(e.target.value) }
                  }))}
                >
              ...
              <div className="settings-row">
                <div>
                  ...
                  <div className="settings-row-desc">Latest time to send emails</div>
                </div>
                <select
                  className="settings-select"
                  ...
                  onChange={(e) => setLocalSettings(prev => ({
                    ...prev,
                    businessHours: { ...prev.businessHours, endHour: parseInt(e.target.value) }
                  }))}
                >
                  {Array.from({ length: 24 }, (_, i) => (
                    <option key={i} value={i}>{i.toString().padStart(2, '0')}:00</option>
            ...
            <button className="btn btn-primary" onClick={handleSaveBusinessHours} disabled={saving}>
              <Save size={16} /> {saving ? 'Saving...' : 'Save Changes'}
            </button>
          </>
        )}
        ...
        {activeSection === 'smart-send' && (
          <>
            <div className="settings-section-header">
              <div className="settings-section-icon" style={{ background: 'rgba(249, 115, 22, 0.1)' }}>
            ...
            <p className="settings-section-desc">
              Optimize email delivery times for maximum engagement. Emails will be scheduled during peak open-rate windows (adjusted for each lead's timezone).
            </p>

            <div className="settings-group">
              <div className="settings-row">
                <div>
                  <div className="settings-row-label">Enable Smart Send Time</div>
                  <div className="settings-row-desc">Automatically optimize send times for better engagement</div>
                </div>
                <button
                  className={`toggle-btn ${localSettings?.smartSendTime?.enabled !== false ? 'on' : 'off'}`}
                  onClick={() => setLocalSettings(prev => ({
                    ...prev,
                    smartSendTime: { ...prev.smartSendTime, enabled: !prev?.smartSendTime?.enabled }
                  }))}
                />
              </div>
            </div>
              ...
              <>
                <div className="settings-group">
                  <div className="settings-group-title">
                    <Flame size={16} color="#f97316" /> Morning Window (Peak Hours)
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div>
                      <label style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Start</label>
                      <select
                        className="settings-select"
                        value={localSettings?.smartSendTime?.morningWindow?.startHour ?? 9}
                        onChange={(e) => setLocalSettings(prev => ({
                          ...prev,
                          smartSendTime: { 
                            ...prev.smartSendTime, 
                            morningWindow: { ...prev.smartSendTime?.morningWindow, startHour: parseInt(e.target.value) }
                          }
                        }))}
                      >
                    ...
                    <div>
                      <label style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>End</label>
                      <select
                        className="settings-select"
                        value={localSettings?.smartSendTime?.morningWindow?.endHour ?? 11}
                        onChange={(e) => setLocalSettings(prev => ({
                          ...prev,
                          smartSendTime: { 
                            ...prev.smartSendTime, 
                            morningWindow: { ...prev.smartSendTime?.morningWindow, endHour: parseInt(e.target.value) }
                          }
                        }))}
                      >
                ...
                <div className="settings-group">
                  ...
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div>
                      ...
                      <select
                        className="settings-select"
                        value={localSettings?.smartSendTime?.afternoonWindow?.startHour ?? 14}
                        onChange={(e) => setLocalSettings(prev => ({
                          ...prev,
                          smartSendTime: { 
                            ...prev.smartSendTime, 
                            afternoonWindow: { ...prev.smartSendTime?.afternoonWindow, startHour: parseInt(e.target.value) }
                          }
                        }))}
                      >
                        {Array.from({ length: 10 }, (_, i) => i + 12).map(h => (
                          <option key={h} value={h}>{h.toString().padStart(2, '0')}:00</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        className="settings-select"
                        value={localSettings?.smartSendTime?.afternoonWindow?.endHour ?? 16}
                        onChange={(e) => setLocalSettings(prev => ({
                          ...prev,
                          smartSendTime: { 
                            ...prev.smartSendTime, 
                            afternoonWindow: { ...prev.smartSendTime?.afternoonWindow, endHour: parseInt(e.target.value) }
                          }
                        }))}
                      >
                ...
                <div className="settings-group">
                  <div className="settings-row">
                    <div>
                      ...
                      <div className="settings-row-desc">Which window to prefer when scheduling</div>
                    </div>
                    <select
                      className="settings-select"
                      value={localSettings?.smartSendTime?.priority || 'morning'}
                      onChange={(e) => setLocalSettings(prev => ({
                        ...prev,
                        smartSendTime: { ...prev.smartSendTime, priority: e.target.value }
                      }))}
                    >
            ...
            <div style={{ 
              ...
              <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
                <strong>How it works:</strong> When an email is scheduled (e.g., 7:30 AM), Smart Send will shift it to the start of your morning window (9:00 AM). 
                Emails between windows get moved to the next one. All times are adjusted for each lead's local timezone.
              </div>
            </div>
            ...
            <button 
              ...
              onClick={async () => {
                setSaving(true);
                try {
                  await updateSettings({ smartSendTime: settings.smartSendTime });
        ...
        {activeSection === 'working-days' && (
          <>
            <div className="settings-section-header">
              <div className="settings-section-icon" style={{ background: 'rgba(34, 197, 94, 0.1)' }}>
            ...
            <p className="settings-section-desc">
              Select which days of the week emails should be sent. Emails won't be scheduled on unchecked days.
            </p>
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map((day, index) => {
                  ...
                  return (
                    ...
                    >
                      {isWorkingDay && <Check size={14} style={{ marginRight: 6 }} />}
                      {day}
                    </button>
                  );
                })}
            ...
            <div style={{ 
              ...
              <span>Working days: <strong>{7 - weekendDays.length}</strong></span>
              <span>Weekend days: <strong>{weekendDays.length}</strong></span>
            </div>
          </>
        )}
        ...
        {activeSection === 'paused-dates' && (
          <>
            ...
            <p className="settings-section-desc">
              Pause email sending on specific dates (holidays, events, etc.). Scheduled emails will be automatically rescheduled.
            </p>
            ...
            <div className="settings-group">
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                ...
                />
                <button className="btn btn-primary" onClick={handlePauseDate} disabled={saving || !newPauseDate}>
            ...
            {pausedDates.length > 0 && (
              <div className="settings-group">
                <div className="settings-group-title">Currently Paused ({pausedDates.length})</div>
        ...
        {activeSection === 'retry' && (
          <>
            <div className="settings-section-header">
              <div className="settings-section-icon" style={{ background: 'rgba(234, 179, 8, 0.1)' }}>
            ...
            <p className="settings-section-desc">
              Configure how the system handles temporary failures, soft bounces, and delivery retries.
            </p>

            <div className="settings-group">
              <div className="settings-row">
                <div>
                  <div className="settings-row-label">Maximum Retry Attempts</div>
                  <div className="settings-row-desc">How many times to retry a failed email</div>
                </div>
                <select
                  className="settings-select"
                  ...
                  onChange={(e) => setLocalSettings(prev => ({
                    ...prev,
                    retry: { ...prev.retry, maxAttempts: parseInt(e.target.value) }
                  }))}
                >
                  {[1, 2, 3, 5, 10, 15].map(n => (
              ...
              <div className="settings-row">
                <div>
                  ...
                  <div className="settings-row-desc">Wait time before retrying soft bounces</div>
                </div>
                <select
                  className="settings-select"
                  ...
                  onChange={(e) => setLocalSettings(prev => ({
                    ...prev,
                    retry: { ...prev.retry, softBounceDelayHours: parseInt(e.target.value) }
                  }))}
                >
        ...
        {activeSection === 'sequences' && (
          <>
            ...
            <p className="settings-section-desc">
              Configure your email automation sequence. Each step represents an email in the followup chain.
            </p>
            ...
            {localSettings?.followups?.sort((a, b) => a.order - b.order).map((followup, index) => (
              <div key={followup.id} style={{ marginBottom: '0.75rem' }}>
                {/* Main Row */}
                <div className="sequence-step" style={{ marginBottom: 0, borderRadius: followup._showConditions ? '10px 10px 0 0' : '10px' }}>
                  <GripVertical size={18} color="var(--text-muted)" style={{ cursor: 'grab' }} />
                  <div className="step-number">{index + 1}</div>
                  <input
                    type="text"
                    value={followup.name}
                    onChange={(e) => handleUpdateFollowup(followup.id, { name: e.target.value })}
                  ...
                  <select
                    ...
                    onChange={(e) => handleUpdateFollowup(followup.id, { templateId: e.target.value })}
                    className="settings-select"
                  ...
                  <select
                    ...
                    onChange={(e) => handleUpdateFollowup(followup.id, { delayDays: parseInt(e.target.value) })}
                    className="settings-select"
                    style={{ minWidth: 'unset' }}
                    disabled={index === 0}
                  >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', gap: '4px' }}>
                    {index > 0 && (
                      <>
                        <button
                          onClick={() => {
                            const updatedFollowups = settings.followups.map(f => 
                              f.id === followup.id ? { ...f, _showConditions: !f._showConditions } : f
                            );
                            setLocalSettings(prev => ({ ...prev, followups: updatedFollowups }));
                          }}
                          title="Configure Conditions"
                          style={{ 
                            background: followup.condition?.type && followup.condition.type !== 'always' 
                              ? 'rgba(168, 85, 247, 0.2)' 
                              : 'rgba(59, 130, 246, 0.1)', 
                            border: 'none', 
                            borderRadius: '8px', 
                            padding: '8px', 
                            cursor: 'pointer', 
                            color: followup.condition?.type && followup.condition.type !== 'always' ? '#a855f7' : '#3b82f6',
                            display: 'flex'
                          }}
                        >
                          <ChevronRight size={16} style={{ 
                            transform: followup._showConditions ? 'rotate(90deg)' : 'rotate(0deg)',
                  ...
                  <div style={{
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => handleUpdateFollowup(followup.id, { 
                          condition: { ...followup.condition, type: e.target.value } 
                        })}
                        className="settings-select"
                        style={{ width: '100%' }}
                      >
                        <option value="always">Always Send</option>
                        <option value="if_opened">📬 If Previous Opened</option>
                        <option value="if_not_opened">📭 If Previous NOT Opened</option>
                        <option value="if_clicked">🖱️ If Previous Clicked</option>
                        <option value="if_not_clicked">🚫 If Previous NOT Clicked</option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => handleUpdateFollowup(followup.id, { 
                          condition: { ...followup.condition, checkStep: e.target.value } 
                        })}
                        className="settings-select"
                        style={{ width: '100%' }}
                        disabled={!followup.condition?.type || followup.condition?.type === 'always'}
                      >
                        <option value="previous">Previous Step</option>
                        {localSettings?.followups?.filter((f, i) => i < index).map(f => (
                    ...
                    <div>
                      ...
                      <select
                        value={followup.condition?.alternativeTemplateId || ''}
                        onChange={(e) => handleUpdateFollowup(followup.id, { 
                          condition: { ...followup.condition, alternativeTemplateId: e.target.value || null } 
                        })}
                        className="settings-select"
                        style={{ width: '100%' }}
                        disabled={!followup.condition?.type || followup.condition?.type === 'always'}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', paddingTop: '20px' }}>
                      <button
                        ...
                        onClick={() => handleUpdateFollowup(followup.id, { 
                          condition: { ...followup.condition, skipIfNotMet: !followup.condition?.skipIfNotMet } 
                        })}
                        disabled={!followup.condition?.type || followup.condition?.type === 'always'}
                        style={{ opacity: (!followup.condition?.type || followup.condition?.type === 'always') ? 0.5 : 1 }}
                      />
                      <div>
                        <div style={{ fontSize: '0.85rem', fontWeight: 500 }}>Skip if not met</div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                          Skip this step entirely instead of using alt template
                        </div>
                      </div>
                    </div>

                    {followup.condition?.type && followup.condition?.type !== 'always' && (
                      <div style={{ gridColumn: '1 / -1', padding: '8px 12px', background: 'rgba(34, 197, 94, 0.1)', borderRadius: '6px', fontSize: '0.8rem', color: '#22c55e' }}>
                        ✓ This step will {followup.condition.skipIfNotMet ? 'be skipped' : followup.condition.alternativeTemplateId ? 'use alternative template' : 'send anyway'} if condition is not met
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="add-step-row">
              ...
              <select
                ...
                onChange={(e) => setNewFollowup(prev => ({ ...prev, delayDays: parseInt(e.target.value) }))}
                className="settings-select"
              >
                {[1, 2, 3, 4, 5, 7, 10, 14, 21, 30].map(n => (
                  <option key={n} value={n}>{n} day{n > 1 ? 's' : ''}</option>
        ...
        {activeSection === 'templates' && (
          <>
            ...
            <p className="settings-section-desc">
              Create and manage email templates. Use variables like {'{{name}}'} to personalize your messages.
            </p>
            ...
            {editingTemplate ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                <div>
                  ...
                  <div style={{ background: 'white', borderRadius: '8px', overflow: 'hidden' }}>
                    <ReactQuill 
                      ...
                      onChange={(val) => setEditingTemplate(prev => ({ ...prev, body: val }))}
                      theme="snow"
                      style={{ height: '280px', marginBottom: '42px' }}
                    />
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Insert variable:</span>
                  {['{{company}}', '{{name}}', '{{email}}', '{{city}}', '{{country}}'].map(v => (
                    <button 
                      key={v}
                      onClick={() => setEditingTemplate(prev => ({ ...prev, body: prev.body + ' ' + v }))}
                      style={{ 
                        background: 'var(--bg-hover)', 
                        padding: '4px 10px', 
                        borderRadius: '6px', 
                ...
                <div style={{ display: 'flex', gap: '12px', marginTop: '0.5rem' }}>
                  <button className="btn btn-primary" onClick={() => handleSaveTemplate(editingTemplate)}>
              ...
              <div>
                {templates.length === 0 ? (
                  <div style={{ 
                    ...
                    <p style={{ fontSize: '0.9rem' }}>Create your first email template to get started.</p>
                  </div>
                ) : (
                  templates.map(t => (
        ...
        {activeSection === 'reports' && (
          <>
            ...
            <p className="settings-section-desc">
              Get an automated summary of your outreach performance delivered to your inbox every week.
            </p>

            <div className="settings-card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <div>
                  <div style={{ fontWeight: 600, color: 'var(--text-primary)', marginBottom: '4px' }}>Enable Weekly Reports</div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Automatically transform analytics into email summaries</div>
                </div>
                <button
                  className={`toggle-btn ${settings.reporting?.enabled ? 'on' : 'off'}`}
                  onClick={() => {
                    const newSettings = { ...settings, reporting: { ...settings.reporting, enabled: !settings.reporting?.enabled } };
              ...
              {settings.reporting?.enabled && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', paddingTop: '1rem', borderTop: '1px solid var(--border-color)' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '8px' }}>Enter email addresses separated by commas</div>
                    <input
                      type="text"
                      className="settings-input"
                      style={{ width: '100%' }}
                      ...
                      onChange={(e) => {
                        const recipients = e.target.value.split(',').map(e => e.trim());
                        const newSettings = { ...settings, reporting: { ...settings.reporting, recipients } };
                        setLocalSettings(newSettings);
                      }}
                    />
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    <div>
                      <label style={{ display: 'block', marginBottom: '8px', fontWeight: 500 }}>Day of Week</label>
                      <select
                        className="settings-select"
                        style={{ width: '100%' }}
                        value={settings.reporting?.dayOfWeek || 1}
                        onChange={(e) => {
                          const newSettings = { ...settings, reporting: { ...settings.reporting, dayOfWeek: parseInt(e.target.value) } };
                          setLocalSettings(newSettings);
                        }}
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const newSettings = { ...settings, reporting: { ...settings.reporting, time: e.target.value } };
                          setLocalSettings(newSettings);
                        }}
                      >
            ...
            <div className="settings-actions">
              <button 
                ...
                disabled={saving}
              >
                {saving ? <div className="loading-spinner small"></div> : <Save size={16} />}
                Save Changes
              </button>
            </div>
          </>
        ...
        {activeSection === 'brevo' && (
          <>
            ...
            <p className="settings-section-desc">
              Configure your Brevo (formerly Sendinblue) API credentials for email delivery.
            </p>
            ...
            <div className="settings-group">
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                ...
                <small style={{ color: 'var(--text-muted)', marginTop: '6px', display: 'block' }}>
                  Get your API key from <a href="https://app.brevo.com/settings/keys/api" target="_blank" rel="noopener noreferrer" style={{ color: '#3b82f6' }}>Brevo Dashboard</a>
                </small>
              </div>

              <div style={{ marginBottom: '1.25rem' }}>
                <label style={{ display: 'block', marginBottom: '8px', fontWeight: 500 }}>From Email</label>
                <input
                  type="email"
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '8px', fontWeight: 500 }}>From Name</label>
                <input
                  type="text"
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button 
                ...
                onClick={async () => {
                  try {
                    const result = await testBrevoConnection(localSettings?.brevo?.apiKey);
                    if (result.success) {
                      showToast?.(`✓ Connected! Account: ${result.account.email}`, 'success');
                    } else {
        ...
        {activeSection === 'danger' && (
          <>
            <div className="settings-section-header">
              <div className="settings-section-icon" style={{ background: 'rgba(239, 68, 68, 0.1)' }}>
            ...
            <div className="danger-zone">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                <div>
                  <div style={{ fontWeight: 600, marginBottom: '4px' }}>Clear Brevo Logs</div>
                ...
                <button 
                  ...
                  onClick={async () => {
                    setConfirmModal({
                      isOpen: true,
                      title: 'Clear Logs',
                      message: 'Are you sure you want to clear all Brevo logs? This cannot be undone.',
                      onConfirm: async () => {
                        const { clearBrevoLogs } = await import('../services/api');
